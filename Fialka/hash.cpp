...
/** Add one more byte to the message buffer and call HashState#hashBlock()
  ...
  */
void hashWriteByte(HashState *hs, uint8_tt byte)
{
	uint8_tt pos; // corrected for endianness
	...
	{
		pos = (uint8_tt)(3 - hs->byte_position_m);
	}
	switch (pos)
	{
	case 0:
		hs->m[hs->index_m] |= ((uint32_tt)byte << 24);
		break;
	case 1:
		hs->m[hs->index_m] |= ((uint32_tt)byte << 16);
		break;
	case 2:
		hs->m[hs->index_m] |= ((uint32_tt)byte << 8);
		break;
	case 3:
	default:
		hs->m[hs->index_m] |= ((uint32_tt)byte);
		break;
	}
	...
	hs->byte_position_m = (uint8_tt)((hs->byte_position_m + 1) & 3);
	if (hs->index_m == 16)
	{
		hs->hashBlock(hs);
		clearM(hs);
...
void hashFinish(HashState *hs)
{
	...
	hashWriteByte(hs, (uint8_tt)0x80);
...
/** Write the hash value into a byte array, respecting endianness.
  ...
  */
void writeHashToByteArray(uint8_tt *out, HashState *hs, bool do_write_big_endian)
{
	uint8_tt i;
