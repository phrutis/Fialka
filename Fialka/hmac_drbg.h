...
typedef struct HMACDRBGStateStruct
{
	...
	uint8_tt key[SHA256_HASH_LENGTH];
	/** This is sometimes called "V" in NIST SP 800-90A This is usually used as
	  * the message/value in HMAC invocations. */
	uint8_tt v[SHA256_HASH_LENGTH];
} HMACDRBGState;

extern void drbgInstantiate(HMACDRBGState *state, const uint8_tt *seed_material, const unsigned int seed_material_length);
extern void drbgReseed(HMACDRBGState *state, const uint8_tt *reseed_material, const unsigned int reseed_material_length);
extern void drbgGenerate(uint8_tt *out, HMACDRBGState *state, const unsigned int requested_bytes, const uint8_tt *additional_input, const unsigned int additional_input_length);
