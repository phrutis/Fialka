...
static uint8_tt *complement_n;
/** The size of #complement_n, in number of bytes. */
static uint8_tt size_complement_n;
...
/** Compare two multi-precision numbers of arbitrary size.
  ...
  */
uint8_tt bigCompareVariableSize(uint8_tt *op1, uint8_tt *op2, uint8_tt size)
{
	uint8_tt i;
	uint8_tt r;
	uint8_tt cmp;
	...
	for (i = (uint8_tt)(size - 1); i < size; i--)
	{
		...
		cmp = (uint8_tt)((((uint16_tt)((int)op2[i] - (int)op1[i])) >> 8) & BIGCMP_GREATER);
		r = (uint8_tt)(((((uint16_tt)(-(int)r)) >> 8) & (r ^ cmp)) ^ cmp);
		cmp = (uint8_tt)((((uint16_tt)((int)op1[i] - (int)op2[i])) >> 8) & BIGCMP_LESS);
		r = (uint8_tt)(((((uint16_tt)(-(int)r)) >> 8) & (r ^ cmp)) ^ cmp);
	}
	return r;
}
...
/** Compare two 32 byte multi-precision numbers.
  ...
  *         and #BIGCMP_LESS if op1 < op2.
  */
uint8_tt bigCompare(BigNum256 op1, BigNum256 op2)
{
...
/** Check if a multi-precision number of arbitrary size is equal to zero.
  ...
  */
uint8_tt bigIsZeroVariableSize(uint8_tt *op1, uint8_tt size)
{
	uint8_tt i;
	uint8_tt r;
	...
	return (uint8_tt)((((uint16_tt)(-(int)r)) >> 8) + 1);
}
...
/** Check if a 32 byte multi-precision number is equal to zero.
  ...
  * \return 1 if op1 is zero, 0 if op1 is not zero.
  */
uint8_tt bigIsZero(BigNum256 op1)
{
...
/** Set prime finite field parameters. The arrays passed as parameters to
  ...
  */
void bigSetField(const uint8_tt *in_n, const uint8_tt *in_complement_n, const uint8_tt in_size_complement_n)
{
	n = (BigNum256)in_n;
	complement_n = (uint8_tt *)in_complement_n;
	size_complement_n = (uint8_tt)in_size_complement_n;
}
...
/** Add (r = op1 + op2) two multi-precision numbers of arbitrary size,
  ...
  */
uint8_tt bigAddVariableSizeNoModulo(uint8_tt *r, uint8_tt *op1, uint8_tt *op2, uint8_tt op_size)
{
	uint16_tt partial;
	uint8_tt carry;
	uint8_tt i;
	...
	for (i = 0; i < op_size; i++)
	{
		partial = (uint16_tt)((uint16_tt)op1[i] + (uint16_tt)op2[i] + (uint16_tt)carry);
		r[i] = (uint8_tt)partial;
		carry = (uint8_tt)(partial >> 8);
	}
	return carry;
}
...
/** Subtract (r = op1 - op2) two multi-precision numbers of arbitrary size,
  ...
  */
uint8_tt bigSubtractVariableSizeNoModulo(uint8_tt *r, uint8_tt *op1, uint8_tt *op2, uint8_tt op_size)
{
	uint16_tt partial;
	uint8_tt borrow;
	uint8_tt i;
	...
	for (i = 0; i < op_size; i++)
	{
		partial = (uint16_tt)((uint16_tt)op1[i] - (uint16_tt)op2[i] - (uint16_tt)borrow);
		r[i] = (uint8_tt)partial;
		borrow = (uint8_tt)((uint8_tt)(partial >> 8) & 1);
	}
	return borrow;
}
...
/** Subtract (r = op1 - op2) two 32 byte multi-precision numbers,
  ...
  * \return 1 if borrow occurred, 0 if no borrow occurred.
  */
uint8_tt bigSubtractNoModulo(BigNum256 r, BigNum256 op1, BigNum256 op2)
{
...
void bigModulo(BigNum256 r, BigNum256 op1)
{
	...
	cmp = (uint8_tt)(bigCompare(op1, n) ^ BIGCMP_LESS);
	cmp = (uint8_tt)((((uint16_tt)(-(int)cmp)) >> 8) + 1);
	lookup[0] = n;
	lookup[1] = zero;
...
void bigAdd(BigNum256 r, BigNum256 op1, BigNum256 op2)
{
	...
	cmp = (uint8_tt)(bigCompare(r, n) ^ BIGCMP_LESS);
	cmp = (uint8_tt)((((uint16_tt)(-(int)cmp)) >> 8) & 1);
	too_big |= cmp;
	lookup[0] = zero;
	lookup[1] = n;
...
void bigShiftRightNoModulo(BigNum256 r, const BigNum256 op1)
{
	...
	for (i = 31; i < 32; i--)
	{
		carry = (uint8_tt)(r[i] & 1);
		r[i] = (uint8_tt)((r[i] >> 1) | (old_carry << 7));
		old_carry = carry;
	}
}
...
/** Multiplies (r = op1 x op2) two multi-precision numbers of arbitrary size,
  ...
  */
void bigMultiplyVariableSizeNoModulo(uint8_tt *r, uint8_tt *op1, uint8_tt op1_size, uint8_tt *op2, uint8_tt op2_size)
{
	uint8_tt cached_op1;
	uint8_tt low_carry;
	uint8_tt high_carry;
	uint16_tt multiply_result16;
	uint8_tt multiply_result_low8;
	uint8_tt multiply_result_high8;
	uint16_tt partial_sum;
	uint8_tt i;
	uint8_tt j;

	memset(r, 0, (uint16_tt)(op1_size + op2_size));
	...
	for (i = 0; i < op1_size; i++)
	{
		...
		{
			multiply_result16 = (uint16_tt)((uint16_tt)cached_op1 * (uint16_tt)op2[j]);
			multiply_result_low8 = (uint8_tt)multiply_result16;
			multiply_result_high8 = (uint8_tt)(multiply_result16 >> 8);
			partial_sum = (uint16_tt)((uint16_tt)r[i + j] + (uint16_tt)multiply_result_low8);
			r[i + j] = (uint8_tt)partial_sum;
			low_carry = (uint8_tt)(partial_sum >> 8);
			partial_sum = (uint16_tt)((uint16_tt)r[i + j + 1] + (uint16_tt)multiply_result_high8 + (uint16_tt)low_carry + (uint16_tt)high_carry);
			r[i + j + 1] = (uint8_tt)partial_sum;
			high_carry = (uint8_tt)(partial_sum >> 8);
		}
#ifdef TEST
		assert(high_carry == 0);
#endif // #ifdef TEST
...
void bigMultiply(BigNum256 r, BigNum256 op1, BigNum256 op2)
{
	...
	while (remaining > 32)
	{
		...
		bigMultiplyVariableSizeNoModulo(\
			...
			&(full_r[32]), (uint8_tt)(remaining - 32));
		...
		remaining = (uint8_tt)(remaining - 32 + size_complement_n);
	}
...
void bigInvert(BigNum256 r, BigNum256 op1)
{
	...
	uint8_tt i;
	uint8_tt j;
	uint8_tt byte_of_n_minus_2;
	uint8_tt bit_of_n_minus_2;
	uint8_tt *lookup[2];
	...
	for (i = 31; i < 32; i--)
	{
		...
		if (i == 0)
		{
			byte_of_n_minus_2 = (uint8_tt)(byte_of_n_minus_2 - 2);
		}
		for (j = 0; j < 8; j++)
		{
			bit_of_n_minus_2 = (uint8_tt)((byte_of_n_minus_2 & 0x80) >> 7);
			byte_of_n_minus_2 = (uint8_tt)(byte_of_n_minus_2 << 1);
