...
#define RELEASE "M-125 (17.01.2022) v4.0"
...
void usage()
{
	printf("Fialka M-125 [OPTIONS...] [TARGETS]\n");
	...
	printf("-c, --check                              : Check the working of the codes\n");
	printf("-n, --nbit                               : Range mode GPU save checkpoints every -n ? minutes (1-10000) Default: false\n");
	printf("-n, --nbit                               : Random mode: What bit range to randomize? -n 1-256 Example -n 252 Default: 252-256 bit\n");
	printf("-z, --zez                                : Random mode: End range for random. Example -n 48 -z 52 (random in 48,49,50,51,52 bit) Default: false\n");
	printf("-s, --seed                               : Random mode: End range for random. Example -n 48 -z 52 (random in 48,49,50,51,52 bit) Default: false\n");
...
int main(int argc, char** argv)
{
	...
	parser.add("-n", "--nbit", true);
	parser.add("-z", "--zez", true);
	parser.add("-d", "--display", true);
	parser.add("-s", "--seed", true);
	if (argc == 1) {
		usage();
		return 0;
	}
	...
	for (unsigned int i = 0; i < args.size(); i++) {
		...
		try {
			...
			else if (optArg.equals("-n", "--nbit")) {
				nbit = std::stoull(optArg.arg);
			}
			else if (optArg.equals("-s", "--seed")) {
				seed = optArg.arg;
			}
			else if (optArg.equals("-z", "--zez")) {
				zez = optArg.arg;
			}
	...
	if (SetConsoleCtrlHandler(CtrlHandler, TRUE)) {
		...
		case (int)SEARCH_MODE_MA:
		case (int)SEARCH_MODE_MX:
			v = new Fialka(inputFile, compMode, seed, zez, searchMode, coinType, gpuEnable, outputFile, useSSE,
				maxFound, rKey, nbit, nbit2, zet, display, rangeStart.GetBase16(), rangeEnd.GetBase16(), should_exit);
			break;
		case (int)SEARCH_MODE_SA:
		case (int)SEARCH_MODE_SX:
			v = new Fialka(hashORxpoint, compMode, seed, zez, searchMode, coinType, gpuEnable, outputFile, useSSE,
				maxFound, rKey, nbit, nbit2, zet, display, rangeStart.GetBase16(), rangeEnd.GetBase16(), should_exit);
			break;
		default:
