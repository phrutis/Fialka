...
static const uint8_tt secp256k1_complement_p[5] = {
0xd1, 0x03, 0x00, 0x00, 0x01};

/** The order of the base point used in secp256k1. */
const uint8_tt secp256k1_n[32] = {
...
static const uint8_tt secp256k1_complement_n[17] = {
...
static const uint8_tt secp256k1_Gx[32]  = {
...
static const uint8_tt secp256k1_Gy[32]  = {
...
static NOINLINE void pointAdd(PointJacobian *p1, PointJacobian *junk, PointAffine *p2)
{
	...
	is_O = (uint8_tt)((((uint16_tt)(-(int)p1->is_point_at_infinity)) >> 8) & 1);
	...
	is_O2 = (uint8_tt)((((uint16_tt)(-(int)p2->is_point_at_infinity)) >> 8) & 1);
	p1 = lookup[is_O2];
	lookup[0] = p1; // p1 might have changed
	...
	cmp_xs = (uint8_tt)(bigCompare(p1->x, s) ^ BIGCMP_EQUAL);
	cmp_xs = (uint8_tt)(((uint16_tt)(-(int)cmp_xs)) >> 8);
	// The following two lines do: "cmp_yt = bigCompare(p1->y, t) == BIGCMP_EQUAL ? 0 : 0xff;".
	cmp_yt = (uint8_tt)(bigCompare(p1->y, t) ^ BIGCMP_EQUAL);
	cmp_yt = (uint8_tt)(((uint16_tt)(-(int)cmp_yt)) >> 8);
	...
	p1->is_point_at_infinity = (uint8_tt)(p1->is_point_at_infinity | (~cmp_xs & cmp_yt & 1));
...
void pointMultiply(PointAffine *p, BigNum256 k)
{
	...
	for (i = 31; i < 32; i--)
	{
		...
		for (j = 0; j < 8; j++)
		{
			...
			one_bit = (uint8_tt)((one_byte & 0x80) >> 7);
			pointAdd(&accumulator, &junk, lookup_affine[one_bit]);
			one_byte = (uint8_tt)(one_byte << 1);
		}
	}
...
void ecdsaSign(BigNum256 r, BigNum256 s, const BigNum256 hash, const BigNum256 private_key)
{
	PointAffine big_r;
	uint8_tt k[32];
	uint8_tt seed_material[32 + SHA256_HASH_LENGTH];
	HMACDRBGState state;
...
/** Serialise an elliptic curve point in a manner which is Bitcoin-compatible.
  ...
  */
uint8_tt ecdsaSerialise(uint8_tt *out, const PointAffine *point, const bool do_compress)
{
	PointAffine temp;
